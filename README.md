...
После запуска сервис будет запущен на http://localhost:8080
...
2. Запуск PostgreSQL 17
3. Применение миграций базы данных
4. Запуск приложения на порту 8080
...
Запуск линтера (опционально, требует установки golangci-lint):
```bash
make lint
# или напрямую
golangci-lint run
...
**Примечание:** Для работы `make lint` необходимо установить [golangci-lint](https://golangci-lint.run/usage/install/). Это опциональный инструмент для разработки, не требуется для запуска сервиса через `docker-compose up`.
